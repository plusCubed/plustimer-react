...
const HistoryDivider = (props: {key: string}) => {
  return (
    <div className={style.dividerItem} key={props.key}>
...
const SolvesList = ({ /*sessions, */solves, expanded, onlyLast, onPenalty, onDelete }: IProps) => {
  ...
  const solveDivs = [];
  let sessionId = -1;
  // Must be in descending timestamp order
  const sessions = [
    /*{id: 123, solves: []}*/
  ];
  for(const solve of solves){
    if(sessionId !== solve.sessionId && sessionId!==-1){
      solveDivs.push(
        <HistoryDivider key={sessionId.toString()+'-divider'}/>
      );
